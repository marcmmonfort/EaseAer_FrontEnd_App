...
export default function ShopOffers() {
  ...
  const renderOfferEntity = (offerItem: OfferEntity) => (
    <View style={styles.productContainer} key={offerItem.uuid}>
        <View style={styles.statusBox}></View>
        <View style={styles.productPack}>
            <View style={styles.productHeader}>
                <Text style={styles.titleNewsText}>{offerItem.priceOffer.toString()}</Text>
            </View>
            <View style={styles.productContent}>
                <Text style={styles.detailsText}>EJEMPLO</Text>
                <QRCode value={offerItem.uuid} size={56} color="#321e29" />
                <Text style={styles.usernameText}>{offerItem.idProductOffer}</Text>
            </View>
        </View>
  ...
  return (
    <ImageBackground style={[styles.backgroundImage, { backgroundColor: '#e9e8e6' }]}>
        ...
        <View style={styles.slideContainer}>
            <Text style={styles.minValueText}>{values[0]*10}</Text>
            ...
            min={0} max={100} step={0.1} allowOverlap={false} />
            <Text style={styles.maxValueText}>{values[1]*10}</Text>
        </View>

        <ScrollView style={styles.scrollStyle}>
            {listOffers
            ? (listOffers
                .filter(offer => offer.idShopOffer.toLowerCase().includes(searchText.toLowerCase()))
                .sort((a, b) => a.idShopOffer.localeCompare(b.idShopOffer))
                .map(renderOfferEntity))
